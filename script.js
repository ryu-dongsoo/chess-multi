class ChessGame {
    ...
    initializeStockfish() {
        ...
        // Stockfish가 로드될 때까지 대기
        const checkStockfish = () => {
            try {
                if (typeof Stockfish !== 'undefined') {
                    console.log('Stockfish 발견! 초기화 시작...');
                    this.stockfish = new Stockfish();
                    this.stockfish.onmessage = (event) => {
                        this.handleStockfishMessage(event.data);
                    };
                    this.stockfish.onerror = (error) => {
                        console.error('Stockfish 오류:', error);
                    };
                    
                    // Stockfish 초기화 - 정말 이길 수 없을 정도로 강력하게 설정
                    this.stockfish.postMessage('uci');
                    this.stockfish.postMessage('setoption name Threads value 8'); // 최대 스레드
                    this.stockfish.postMessage('setoption name Hash value 2048'); // 최대 해시
                    ...
                    this.stockfish.postMessage('setoption name Move Overhead value 0');
                    this.stockfish.postMessage('setoption name Minimum Thinking Time value 0');
                    this.stockfish.postMessage('setoption name Slow Mover value 100');
                    this.stockfish.postMessage('setoption name UCI_Chess960 value false');
                    this.stockfish.postMessage('setoption name UCI_AnalyseMode value false');
                    this.stockfish.postMessage('isready');
                    console.log('Stockfish AI 엔진 초기화 완료 - 정말 이길 수 없을 정도로 강력함!');
                } else {
                    retryCount++;
                    console.log(`Stockfish 로딩 중... (${retryCount}/${maxRetries}) 다시 시도합니다.`);
                    
                    if (retryCount < maxRetries) {
                        setTimeout(checkStockfish, 1000); // 1초 후 다시 시도
                    } else {
                        console.log('Stockfish 로딩 실패. 대안 AI를 사용합니다.');
                        // Stockfish 대신 기존 AI 사용
                        this.stockfishReady = false;
                    }
                }
            } catch (error) {
                console.error('Stockfish 초기화 오류:', error);
                retryCount++;
                if (retryCount < maxRetries) {
                    setTimeout(checkStockfish, 2000); // 2초 후 다시 시도
                } else {
                    console.log('Stockfish 초기화 실패. 대안 AI를 사용합니다.');
                    this.stockfishReady = false;
                }
            }
        };
        ...
        // DOMContentLoaded 이벤트에서도 체크
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', checkStockfish);
        }
        
        // window load 이벤트에서도 체크
        window.addEventListener('load', checkStockfish);
    }
    ...
    makeMove(fromRow, fromCol, toRow, toCol) {
        ...
        // 온라인 모드에서 localStorage에 게임 상태 업데이트
        if (this.gameMode === 'online-player') {
    ...
    makeAIMove() {
        ...
        // Stockfish AI 사용
        if (this.aiDifficulty === 'stockfish' && this.stockfish && this.stockfishReady) {
    ...
    createRoom() {
        ...
        localStorage.setItem(`chess_room_${roomId}`, JSON.stringify(roomData));
        localStorage.setItem('chess_current_room', roomId);
        localStorage.setItem('chess_player_name', playerName);
        localStorage.setItem('chess_player_color', 'white'); // 방 생성자는 흰색
        
        this.setGameMode('online-player');
        this.playerColor = 'white';
        this.currentPlayer = 'white';
        this.updateConnectionStatus('방 생성됨 - 상대방 대기 중', true);
        
        console.log('방을 생성했습니다:', roomId);
    ...
    joinRoom() {
        ...
        const roomDataStr = localStorage.getItem(`chess_room_${roomId}`);
        if (!roomDataStr) {
            alert('존재하지 않는 방입니다.');
            return;
        }
        ...
        // 두 번째 플레이어 추가 (검은색 팀)
        roomData.players.push({name: playerName, color: 'black', joinedAt: Date.now()});
        ...
        localStorage.setItem('chess_current_room', roomId);
        localStorage.setItem('chess_player_name', playerName);
        localStorage.setItem('chess_player_color', 'black'); // 참여자는 검은색
        
        this.setGameMode('online-player');
        this.playerColor = 'black';
        this.currentPlayer = 'white'; // 게임은 항상 흰색부터 시작
        this.loadGameState(roomData.gameState);
        
        this.updateConnectionStatus('방에 참여됨 - 게임 시작!', true);
        
        console.log('방에 참여했습니다:', roomId);
        console.log('참여자 팀 할당:', playerName, '-> 검은색');
        alert(`방에 참여했습니다! 당신은 검은색 플레이어입니다.`);
    ...
    randomMatch() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key && key.startsWith('chess_room_')) {
                const roomData = JSON.parse(localStorage.getItem(key));
                if (roomData.players.length === 1 && roomData.status === 'waiting') {
        ...
        if (availableRooms.length === 0) {
            ...
            document.getElementById('room-id').value = randomRoom.id;
            console.log('랜덤 매칭으로 방에 참여합니다:', randomRoom.id);
            this.joinRoom();
        }
    }
    ...
    checkForUpdates() {
        const roomId = localStorage.getItem('chess_current_room');
        if (!roomId) return;
        ...
        const roomData = JSON.parse(roomDataStr);
        const playerName = localStorage.getItem('chess_player_name');
        ...
        if (roomData.gameState) {
            ...
            const roomStateStr = JSON.stringify(roomData.gameState);
            
            if (roomStateStr !== currentStateStr) {
        ...
        // 플레이어 수 및 상태 확인
        if (roomData.players.length === 2 && roomData.status === 'playing') {
            this.updateConnectionStatus('게임 진행 중', true);
            
            // 상대방 정보 표시
            const opponent = roomData.players.find(p => p.name !== playerName);
            if (opponent) {
                console.log(`상대방: ${opponent.name} (${opponent.color})`);
            }
        } else if (roomData.players.length === 1) {
            this.updateConnectionStatus('상대방 대기 중', true);
        }
        
        // 연결 상태 업데이트
        const lastUpdate = roomData.lastUpdate || 0;
        const now = Date.now();
        if (now - lastUpdate > 30000) { // 30초 이상 업데이트 없으면 연결 끊김
            this.updateConnectionStatus('상대방 연결 끊김', false);
        }
    }
